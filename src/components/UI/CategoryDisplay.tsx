  ...
  return (
    <div className="pt-8">
      ...
      <div className="flex gap-4 overflow-x-auto scrollbar">
        {category?.map((cat) => (
          <div key={cat?.id} className="w-30 p-4">
            <Card
              key={cat?.id}
              onPress={() => handleCategory(cat?.name)}
              className="flex flex-col items-center cursor-pointer p-4 rounded-lg shadow hover:bg-gray-100 transition w-[150px] h-[150px]"
            >
              <Image
                src={cat?.img}
                width={200}
